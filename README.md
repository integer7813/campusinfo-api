...
Auto-Deployment: GitHub에 소스 푸시 시, 서버에 설치된 Runner가 이를 감지하여 자동으로 빌드 및 배포를 수행합니다.

Containerization: API 서버와 Cloudflare Tunnel을 개별 컨테이너로 격리하여 관리하며, 서버 재부팅 시에도 자동으로 복구됩니다.

Secure Access: 고정 IP나 포트 포워딩 없이 Cloudflare Tunnel을 사용하여 보안성을 극대화하였으며, https://api.integer7813.cloud를 통해 안전하게 서빙됩니다.
...
Swagger UI: https://api.integer7813.cloud/docs (실시간 테스트 가능)

Redoc: https://api.integer7813.cloud/redoc (깔끔한 문서 열람)
...
JSON
{
  ...
  "server_time_kst": "2026-05-10 19:16:04 KST",
  "data": { ... }
}
🔑 환경 변수 설정 (Secrets)
배포 자동화를 위해 GitHub Repository의 Settings > Secrets > Actions에 아래 항목이 등록되어야 합니다.
